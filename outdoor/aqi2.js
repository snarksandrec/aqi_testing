function getData() {
  fetch("/aqi/aqi2.json").then(response => {
...
function updateHtml(data) {
  let o_aqiPm25 = calcAQIpm25(data.pm25);
  let o_aqiPm10 = calcAQIpm10(data.pm10);

  //update HTML
  document.getElementById("o_time").innerHTML = data.time;
  document.getElementById("o_o_aqiPm25").innerHTML = o_aqiPm25;
  document.getElementById("o_o_aqiPm10").innerHTML = o_aqiPm10;
  ...
  //set colors
  colorsPm25 = getColor(o_aqiPm25);
  colorsPm10 = getColor(o_aqiPm10);
...
function calcAQIpm25(pm25) {
  let pm1 = 0;
	...
	if (pm25 >= pm1 && pm25 <= pm2) {
		o_aqiPm25 = ((aqi2 - aqi1) / (pm2 - pm1)) * (pm25 - pm1) + aqi1;
	} else if (pm25 >= pm2 && pm25 <= pm3) {
		o_aqiPm25 = ((aqi3 - aqi2) / (pm3 - pm2)) * (pm25 - pm2) + aqi2;
	} else if (pm25 >= pm3 && pm25 <= pm4) {
		o_aqiPm25 = ((aqi4 - aqi3) / (pm4 - pm3)) * (pm25 - pm3) + aqi3;
	} else if (pm25 >= pm4 && pm25 <= pm5) {
		o_aqiPm25 = ((aqi5 - aqi4) / (pm5 - pm4)) * (pm25 - pm4) + aqi4;
	} else if (pm25 >= pm5 && pm25 <= pm6) {
		o_aqiPm25 = ((aqi6 - aqi5) / (pm6 - pm5)) * (pm25 - pm5) + aqi5;
	} else if (pm25 >= pm6 && pm25 <= pm7) {
		o_aqiPm25 = ((aqi7 - aqi6) / (pm7 - pm6)) * (pm25 - pm6) + aqi6;
	} else if (pm25 >= pm7 && pm25 <= pm8) {
		o_aqiPm25 = ((aqi8 - aqi7) / (pm8 - pm7)) * (pm25 - pm7) + aqi7;
	}
	return o_aqiPm25.toFixed(0);
}
...
function calcAQIpm10(pm10) {
	...
	if (pm10 >= pm1 && pm10 <= pm2) {
		o_aqiPm10 = ((aqi2 - aqi1) / (pm2 - pm1)) * (pm10 - pm1) + aqi1;
	} else if (pm10 >= pm2 && pm10 <= pm3) {
		o_aqiPm10 = ((aqi3 - aqi2) / (pm3 - pm2)) * (pm10 - pm2) + aqi2;
	} else if (pm10 >= pm3 && pm10 <= pm4) {
		o_aqiPm10 = ((aqi4 - aqi3) / (pm4 - pm3)) * (pm10 - pm3) + aqi3;
	} else if (pm10 >= pm4 && pm10 <= pm5) {
		o_aqiPm10 = ((aqi5 - aqi4) / (pm5 - pm4)) * (pm10 - pm4) + aqi4;
	} else if (pm10 >= pm5 && pm10 <= pm6) {
		o_aqiPm10 = ((aqi6 - aqi5) / (pm6 - pm5)) * (pm10 - pm5) + aqi5;
	} else if (pm10 >= pm6 && pm10 <= pm7) {
		o_aqiPm10 = ((aqi7 - aqi6) / (pm7 - pm6)) * (pm10 - pm6) + aqi6;
	} else if (pm10 >= pm7 && pm10 <= pm8) {
		o_aqiPm10 = ((aqi8 - aqi7) / (pm8 - pm7)) * (pm10 - pm7) + aqi7;
	}
	return o_aqiPm10.toFixed(0);
}
